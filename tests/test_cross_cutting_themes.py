    ...
    return {
        1: pd.DataFrame(
            [
                {
                    "topic_id": "A",
                    "topic": "Employment Support: Help finding and maintaining jobs",
                },
                {
                    "topic_id": "B",
                    "topic": "Financial Assistance: Support with benefits and costs",
                },
                {
                    "topic_id": "C",
...
def test_format_themes_for_prompt(sample_questions_themes):
    ...
    assert (
        "Question 1, Theme A: Employment Support - Help finding and maintaining jobs"
        in result
    )
    assert (
        "Question 2, Theme B: Employment Training - Skills development programs"
        in result
    )
...
def test_step1_identify_cross_cutting_themes(mock_load_prompt, mock_llm):
    ...
    themes_data = (
        "Question 1, Theme A: Employment Support\nQuestion 2, Theme B: Training"
    )
    result = _step1_identify_cross_cutting_themes(
        themes_data, mock_llm, "system prompt"
    )
...
def test_step2_review_unused_themes(mock_load_prompt, mock_llm):
    ...
    result = _step2_review_unused_themes(
        initial_groups, unused_themes, mock_llm, "system prompt"
    )
...
@patch(
    "themefinder.advanced_tasks.cross_cutting_themes._step1_identify_cross_cutting_themes"
)
def test_analyze_cross_cutting_themes_full_flow(
    mock_step1, mock_step2, sample_questions_themes, mock_llm
):
    ...
    result = analyze_cross_cutting_themes(
        sample_questions_themes, mock_llm, "system prompt", min_themes=2
    )
...
def test_analyze_cross_cutting_themes_empty_input(mock_llm):
    ...
    result = analyze_cross_cutting_themes({}, mock_llm, "system prompt", min_themes=3)
...
def test_analyze_cross_cutting_themes_no_unused_themes(
    mock_step1, sample_questions_themes, mock_llm
):
    ...
    result = analyze_cross_cutting_themes(
        sample_questions_themes, mock_llm, "system prompt", min_themes=5
    )
...
def test_cross_cutting_theme_model_validation():
    ...
    with pytest.raises(
        ValueError,
        match="Cross-cutting theme must include at least 2 constituent themes",
    ):
        CrossCuttingTheme(
            name="Test Theme",
    ...
    with pytest.raises(
        ValueError, match="Duplicate question_number and theme_key combinations found"
    ):
        CrossCuttingTheme(
            name="Test Theme",
            description="Test description",

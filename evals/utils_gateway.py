...
async def discover_chat_models() -> list[GatewayModel]:
    """Fetch and combine gateway model info into the eval-ready model list.

    Excludes only models whose most recent, non-stale health check reports
    "unhealthy". Models with no recent health data are included as
    "unknown" — absence of evidence isn't evidence of a problem.
    """
    ...
    return [
        GatewayModel(name=name, family=derive_family(name), health=health)
        for name in chat_names
        if (health := health_by_name.get(name, "unknown")) != "unhealthy"
    ]

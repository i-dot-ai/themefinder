...
class CrossCuttingThemesAgent:
    ...
    def analyze(self, questions_themes: Dict[int, Dict]) -> List[Dict]:
        ...
        logger.info(
            f"Target: {self.max_groups} groups, avg size {self.target_group_size}"
        )
        ...
        logger.info(
            f"Final: {len(self.cross_cutting_groups)} groups, "
    ...
    def _validate_input(self, questions_themes: Dict[int, Dict]) -> None:
        ...
        for question_num, question_data in questions_themes.items():
            ...
            missing_columns = [
                col for col in required_columns if col not in themes_df.columns
            ]
            if missing_columns:
                raise KeyError(
                    f"Question {question_num} themes missing columns: {missing_columns}"
                )
    ...
    def _step1_initial_analysis(self) -> None:
        ...
        prompt = prompt_template.format(
            system_prompt=self.system_prompt, themes_data=themes_data_str
        )
    ...
    def _consolidate_small_groups(self) -> None:
        ...
        small_groups = [
            g
            for g in self.cross_cutting_groups
            if len(g["themes"]) < self.target_group_size
        ]
    ...
    def _heuristic_consolidation(self, small_groups: List[Dict]) -> None:
        ...
        for i, group1 in enumerate(small_groups):
            ...
            for j, group2 in enumerate(small_groups[i + 1 :], i + 1):
                ...
                if (
                    combined_size <= self.target_group_size * 1.5
                ):  # Don't create oversized groups
        ...
        self.cross_cutting_groups = [
            g
            for i, g in enumerate(self.cross_cutting_groups)
    ...
    def _groups_are_related(self, group1: Dict, group2: Dict) -> bool:
        ...
        return len(name_overlap) > 1 or (
            len(name_overlap) > 0 and len(desc_overlap) > 2
        )
    ...
    def _merge_groups(self, group1: Dict, group2: Dict) -> Dict:
        ...
        if "public ownership" in name1_lower and "public ownership" in name2_lower:
        ...
        elif "consumer" in name1_lower and (
            "affordability" in name2_lower or "pricing" in name2_lower
        ):
            merged_name = "Consumer Protection and Affordability"
        elif "environment" in name1_lower and (
            "health" in name2_lower or "sustainability" in name2_lower
        ):
            ...
            merged_name = (
                group1["name"]
                if len(group1["themes"]) >= len(group2["themes"])
                else group2["name"]
            )
        ...
        if any(
            word in desc1.lower() and word in desc2.lower()
            for word in [
                "accountability",
                "transparency",
                "public",
            ...
            merged_desc = (
                f"{desc1} This encompasses related themes including {desc2.lower()}"
            )
        else:
    ...
    def _review_and_redistribute(self) -> None:
        ...
        existing_themes_str = "\n".join(
            [
                ...
                + ", ".join(
                    [f"Q{t['question_number']}-{t['theme_key']}" for t in cc["themes"]]
                )
    ...
    def _discover_additional_patterns(self) -> None:
        ...
        logger.info(
            f"Discovering additional patterns in {len(self.unused_themes)} unused themes"
        )
            ...
            for new_pattern in result.cross_cutting_themes:
                if len(new_pattern.themes) >= self.min_themes:
                    ...
                    unique_questions = set(
                        t.question_number for t in new_pattern.themes
                    )
    ...
    def _post_discovery_consolidation(self) -> None:
        ...
        logger.info(
            f"Post-discovery consolidation: checking {len(self.cross_cutting_groups)} groups for semantic similarity"
        )
        ...
        for i, group1 in enumerate(self.cross_cutting_groups):
            ...
            for j, group2 in enumerate(self.cross_cutting_groups[i + 1 :], i + 1):
                ...
                if self._groups_are_related(group1, group2):
                    ...
                    combined_size = len(group1["themes"]) + len(group2["themes"])
                    if (
                        combined_size <= self.target_group_size * 2
                    ):  # Don't create overly large groups
        ...
        if groups_to_merge:
            ...
            for i, j, group1, group2 in groups_to_merge:
                ...
                logger.info(
                    f"Merged '{group1['name']}' with '{group2['name']}' → '{merged['name']}'"
                )
...
"""
    ...
    def _validate_and_fix_rule_violations(self) -> None:
        ...
        for group in self.cross_cutting_groups[:]:  # Create copy to allow modification
            ...
            # Find violations
            violated_questions = {
                q: themes for q, themes in question_themes.items() if len(themes) > 1
            }

            if violated_questions:
                violations_found += sum(
                    len(themes) - 1 for themes in violated_questions.values()
                )
                groups_fixed += 1

                logger.info(
                    f"Fixing rule violations in '{group['name']}': "
        ...
        if violations_found > 0:
            logger.info(
                f"Fixed {violations_found} rule violations across {groups_fixed} groups"
            )
            ...
            logger.info(
                "No rule violations found - all groups comply with cross-cutting theme rule"
            )
        ...
        try:
            ...
            for addition in result.additions:
                if addition.question_number in violated_questions:
                    ...
                    original_theme_keys = {
                        t["theme_key"]
                        for t in violated_questions[addition.question_number]
                    }
    ...
    def _create_violation_fix_prompt(
        self, group: Dict, violated_questions: Dict
    ) -> str:
        ...
        for q_num, themes in violated_questions.items():
            theme_details = []
            for theme in themes:
                details = self._get_theme_details_for_prompt(
                    theme["question_number"], theme["theme_key"]
                )
                theme_details.append(f"    • {theme['theme_key']}: {details['label']}")
                theme_details.append(f"      {details['description']}")

            violation_details.append(
                f"  Question {q_num} ({len(themes)} themes - VIOLATION):"
            )
...
"""
    ...
    def _get_theme_details_for_prompt(
        self, question_number: int, theme_key: str
    ) -> Dict[str, str]:
        """Get theme details for prompt generation."""
        for theme in self.formatted_themes:
            if (
                theme["question_number"] == question_number
                and theme["theme_key"] == theme_key
            ):
                return {
    ...
    def _calculate_quality_metrics(self) -> None:
        ...
        self.average_group_size = (
            sum(self.group_sizes) / len(self.group_sizes) if self.group_sizes else 0
        )

    def _get_current_coverage(self) -> float:
        """Calculate current theme coverage percentage."""
        return (
            len(self.used_themes) / self.total_themes if self.total_themes > 0 else 0.0
        )
    ...
    def _calculate_quality_score(self) -> float:
        ...
        size_score = (
            1.0
            - abs(self.average_group_size - self.target_group_size)
        ...
        group_count_score = (
            1.0 - abs(len(self.cross_cutting_groups) - ideal_groups) / ideal_groups
        )
        ...
        if len(self.group_sizes) > 1:
            variance = sum(
                (s - self.average_group_size) ** 2 for s in self.group_sizes
            ) / len(self.group_sizes)
            std_dev = variance**0.5
            uniformity_score = (
                1.0 - (std_dev / self.average_group_size)
                if self.average_group_size > 0
                else 0
            )
        ...
        # Weighted average
        quality_score = (
            size_score * 0.4 + group_count_score * 0.3 + uniformity_score * 0.3
        )

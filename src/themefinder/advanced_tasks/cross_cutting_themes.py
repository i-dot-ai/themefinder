...
def analyze_cross_cutting_themes(
    ...
    logger.info(f"Step 1 complete: {len(initial_groups)} initial cross-cutting themes identified")
    ...
    filtered_groups = [
        ...
        if len(group["themes"]) >= min_themes
    ]
    ...
    for question_num, themes_df in questions_themes.items():
        for _, row in themes_df.iterrows():
            ...
            if "topic" in themes_df.columns:
                ...
                topic_description = topic_parts[1].strip() if len(topic_parts) > 1 else topic_label
            else:
...
def _step1_identify_cross_cutting_themes(
    ...
    for cc_theme in result.cross_cutting_themes:
        group = {
            ...
            "themes": [
                {
                    "question_number": theme.question_number,
...
def _step2_review_unused_themes(
    ...
    existing_themes_str = "\n".join([
        ...
        ", ".join([f"Q{t['question_number']}-{t['theme_key']}" for t in group['themes']])
